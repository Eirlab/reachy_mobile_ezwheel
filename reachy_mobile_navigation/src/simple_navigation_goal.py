"""
Implement class simple navigation goal to send goal to move_base and cancel goal
"""
import math
...
class SimpleNavigationGoals:
    """
    Interface with the move_base actionlb. Allows to set a position goal (x, y, theta) that will be followed by a
    robot, using a predefined GlobalPlanner and LocalPlanner
    Raises:
        InitException --
    """
    ...
    def __init__(self, hmi_wrapper=None):
        ...
        success = self.move_base.wait_for_server(rospy.Duration(120))
        if not success:
            rospy.logerr(
        ...
        rospy.loginfo("Server has responded")
        ...
        # A (x, y, theta) list saving the current position that is feedback from AMCL. This is only updated while
        # the movement has not finished
    ...
    def _compute_remaining_distance(self, path):
        ...
        while i < len(path.poses):
            tmp += math.sqrt(math.pow(path.poses[i - 1].pose.position.x - path.poses[i].pose.position.x, 2) + math.pow(
    ...
    def go_to(self, x, y, theta, wait_for_result=60, frame="map", blocking=True):
        """
        Sets  x, y, theta goal for the move_base to follow.
        :param x: {float} -- In meters
        :param y: {float} -- In meters
        :param theta: {float} -- In rads
        :param wait_for_result:  Time in seconds that the robot has to arrive to the goal before it's considered a
             failure (default: {60})
        :param frame: frame considered for the coordinates' system (default: {"map"})
        :param blocking:
